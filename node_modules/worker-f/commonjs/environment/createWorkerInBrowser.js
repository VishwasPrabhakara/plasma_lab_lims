...
exports["default"] = createWorkerInBrowser;
...
function createWorkerInBrowser(javascriptCode, getFromCache, setInCache, onError, onOutput) {
  ...
  worker.onmessage = function (event) {
    var data = event.data;
    var errorData = data ? data[ERROR_MESSAGE_PROPERTY_NAME] : undefined;
    if (errorData) {
      var error = new Error(errorData[0]);
...
function createWorkerCodeUrl(javascriptCode) {
  return URL.createObjectURL(new Blob([javascriptCode + ';' + JAVASCRIPT_CODE_ADDITIONAL], {
...
'self.onmessage = function(evt) {' + 'onMessage(evt.data)' + '}' + ';' +
// Reports an error to the main thread.
'function onErr(err_, msg) {' + 'var err = err_ instanceof Error ? err_ : new Error(msg)' + ';' + 'postMessage({' + ERROR_MESSAGE_PROPERTY_NAME + ':' + '[' +
...
'Object.keys(err).map(' + 'function(k){' + 'return[k,err[k]]' + '}' + ')' + ')' + '})' + '}' + ';' +
...
//# sourceMappingURL=createWorkerInBrowser.js.map